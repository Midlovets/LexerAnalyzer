...
TRACE = False
IS_PRE_PARSE = False  # <--- НОВИЙ ГЛОБАЛЬНИЙ ПРАПОРЕЦЬ
...
def get_symb():
    ...
    # Додаємо +1 до numRow, оскільки індекси таблиці починаються з 1
    if numRow not in tableOfSymb:
        fail_parse("неочікуваний кінець програми (невірний індекс)", numRow)
...
def fail_parse(msg, data):
    if msg == "неочікуваний кінець програми":
        ...
        if row > 1 and (row - 1) in tableOfSymb:
...
def fail_semantic(msg, data):
    ...
    if msg == "повторне оголошення змінної":
        ...
        # --- ЗМІНА ---
        # Під час 2-го проходу це нормально, просто ігноруємо
        if not IS_PRE_PARSE:
            print(
                f"    [Semantic] Повторне оголошення функції '{func_name}' (рядок {numLine}) проігноровано (вже додано на 1-му проході)")
            return
        # --- КІНЕЦЬ ЗМІНИ ---
...
def add_func_to_table(func_name, params, return_type, numLine):
    ...
    if func_name in tableOfFunc:
        ...
        return  # Дозволяємо "повторне оголошення" на 2-му проході
...
def skip_block():
    """Пропускає цілий блок { ... } під час 1-го проходу"""
    global numRow
    numLine, lex, tok = get_symb()

    if lex != "{" or tok != "brackets_op":
        fail_parse("несумісність токенів (очікувався '}')", (numLine, lex, tok, "{", "brackets_op"))
    ...
    while numRow <= len_tableOfSymb:
        ...
        if lex == "{" and tok == "brackets_op":
            ...
            if brace_level == 0:
                numRow += 1  # Пропустити '}'
                return  # Знайшли кінець блоку

        numRow += 1

    fail_parse("Незакритий блок { ... } під час пропуску тіла", (numLine, lex, tok))


def skip_expression_body():
    """Пропускає тіло-вираз (= ...) під час 1-го проходу"""
    global numRow
    # Просто шукаємо наступний 'fun' на верхньому рівні або кінець файлу.
    # Це спрощення, яке припускає, що 'fun' не може бути всередині виразу.
    while numRow <= len_tableOfSymb:
        numLine, lex, tok = get_symb()
        if lex == "fun" and tok == "keyword":
            # Знайшли початок наступної функції, не "споживаємо" токен
            return
        numRow += 1
...
def parse_program():
    global numRow, IS_PRE_PARSE
    try:
        # --- ПОЧАТОК ЗМІН: ПЕРШИЙ ПРОХІД ---
        IS_PRE_PARSE = True
        print("\n--- ПОЧАТОК ПЕРШОГО ПРОХОДУ (ЗБІР СИГНАТУR) ---")

        original_numRow = 1  # Завжди починаємо з 1

        temp_row = 1
        while temp_row <= len_tableOfSymb:
            numRow = temp_row  # Встановлюємо глобальний лічильник
            if numRow not in tableOfSymb: break  # Кінець файлу

            numLine, lex, tok = get_symb()
            if lex == "fun" and tok == "keyword":
                parse_function_declaration()  # Викличе модифіковану версію
                temp_row = numRow  # Оновлюємо temp_row до позиції ПІСЛЯ парсингу
            else:
                temp_row += 1  # Пропустити все інше

        print("--- ПЕРШИЙ ПРОХІД ЗАВЕРШЕНО ---")

        # --- ДРУГИЙ ПРОХІД (ПОВНИЙ АНАЛІЗ) ---
        numRow = original_numRow  # Скидаємо для другого проходу
        IS_PRE_PARSE = False

        indent = next_ident()
        print(f"\n{indent}parse_program(): (ДРУГИЙ ПРОХІД)")
        ...
        temp_row = 1
        while temp_row <= len_tableOfSymb:
            # Перевірка, чи є наступний токен
            if (temp_row + 1) in tableOfSymb:
                if tableOfSymb[temp_row][1] == "fun" and tableOfSymb[temp_row + 1][1] == "main":
                    found_main = True
                    main_start_row = temp_row
                    break
            temp_row += 1
        ...
        # 2. Розібрати всі функції ДО 'main'
        numRow = original_numRow
        while numRow < main_start_row:
            if numRow not in tableOfSymb: break
            numLine, lex, tok = get_symb()
            if lex == "fun" and tok == "keyword":
                parse_function_declaration()  # Повний парсинг
            else:
                numRow += 1  # Пропустити

        # 3. Розібрати 'main'
        numRow = main_start_row
        parse_main_function()  # Повний парсинг 'main'

        # 4. Розібрати всі функції ПІСЛЯ 'main'
        while numRow <= len_tableOfSymb:
            if numRow not in tableOfSymb: break
            numLine, lex, tok = get_symb()
            if lex == "fun" and tok == "keyword":
        ...
        if numRow == len_tableOfSymb + 1:
            ...
            print(f"\nПОПЕРЕДЖЕННЯ: Аналізатор не дійшов до кінця файлу. numRow={numRow}, len={len_tableOfSymb + 1}")
            return True  # Вважаємо це успіхом
        ...
        prev_ident()
        IS_PRE_PARSE = False  # Переконаємося, що прапорець скинуто
...
def parse_main_function():
    ...
    # 'main' додається тільки на 2-му проході
...
def parse_function_declaration():
    global numRow, currentFunction, IS_PRE_PARSE
    indent = next_ident()
    if IS_PRE_PARSE:
        print(f"{indent}pre_parse_function_declaration():")
    else:
    ...
    # 'main' обробляється окремо
    if lex == "main" and tok == "keyword":
        if IS_PRE_PARSE:
            # Під час pre-pass ми повністю ігноруємо main
            print(f"{indent}Пропуск 'main' під час 1-го проходу.")
            numRow += 1  # пропустити 'main'
            numRow += 1  # пропустити '('
            numRow += 1  # пропустити ')'
            if get_symb()[1] == '{':
                skip_block()
            prev_ident()
            return True  # Завершуємо
        else:
            # 'main' має парситися ТІЛЬКИ через parse_main_function
            fail_parse("несумісність токенів (fun main поза логікою)",
                       (numLine, lex, tok, "identifier (not main)", "identifier"))
    ...
    numLine, lex, tok = get_symb()
    if lex != ")":
        params = parse_params()  # parse_params змінено
    parse_token(")", "brackets_op")
    ...
    # --- ЗМІНА ---
    # Пропускаємо тіло функції, якщо це pre-pass
    if IS_PRE_PARSE:
        numLine, lex, tok = get_symb()
        if lex == "=" and tok == "assign_op":
            numRow += 1
            print(f"{indent}Пропуск тіла-виразу (=) для '{function_name}'")
            skip_expression_body()
        elif lex == "{" and tok == "brackets_op":
            print(f"{indent}Пропуск тіла-блоку ({{...}}) для '{function_name}'")
            skip_block()
        else:
            fail_parse("несумісність токенів", (numLine, lex, tok, "= або {", "(assign_op або brackets_op)"))

        currentFunction = None  # Скидаємо контекст
        prev_ident()
        return True  # Завершуємо
    # --- КІНЕЦЬ ЗМІНИ ---

    # Цей код виконається ТІЛЬКИ під час головного (другого) проходу
...
def parse_param():
    global numRow, IS_PRE_PARSE  # <--- ДОДАНО IS_PRE_PARSE
    indent = next_ident()
    ...
    if lex == "=" and tok == "assign_op":
        ...
        # --- ЗМІНА ---
        if IS_PRE_PARSE:
            # Просто пропускаємо вираз
            print(f"{indent}Пропуск виразу за замовчуванням для '{param_name}'")
            level = 0
            while numRow <= len_tableOfSymb:
                l, x, t = get_symb()
                if x == '(' or x == '{' or x == '[':
                    ...
                    level -= 1
                elif (x == ',' or x == ')') and level == 0:
                    break  # Знайшли кінець виразу
                numRow += 1
                if numRow > len_tableOfSymb:
                    fail_parse("незакритий вираз за замовчуванням", (l, x, t))
        else:
            # Повноцінний парсинг
            default_type = parse_expression()
            if default_type != param_type:
                if not (default_type == 'Real' and default_type == 'Int'):
...
def parse_block(is_function_block=False, function_name=None):
    ...
    while True:
        ...
        current_line, lex, tok = get_symb()
        if lex == "}" and tok == "brackets_op":
            break  # Знайшли кінець блоку
...
def parse_for_range():
    ...
    if lex == ".." and tok == "punct":
        ...
        if range_start_type != 'Int' or range_end_type != 'Int':
            print(
                f"    [Semantic] ПОПЕРЕДЖЕННЯ: Діапазон має бути Int..Int, отримано {range_start_type}..{range_end_type}")
        numLine, lex, tok = get_symb()
        ...
        if range_start_type != 'Int' or range_end_type != 'Int':
            print(
                f"    [Semantic] ПОПЕРЕДЖЕННЯ: Діапазон має бути Int downTo Int, отримано {range_start_type} downTo {range_end_type}")
        numLine, lex, tok = get_symb()
...
def parse_when_condition(when_expr_type):
    ...
    if cond_type != when_expr_type:
        print(
            f"    [Semantic] ПОПЕРЕДЖЕННЯ: Тип умови when ({cond_type}) не співпадає з типом виразу ({when_expr_type})")
